...
void AA_ItemActor::OnRep_RootItemID()
{
	OnRepRootItemIDChanged();
}
...
void AA_ItemActor::SpawnedFromDropEvent_Implementation(UAC_Inventory* PreviousOwner)
{
	if(Inventory && Inventory->ContainerSettings.IsValidIndex(0) && Inventory->ContainerSettings[0].Items.IsValidIndex(0))
	{
		if(Inventory->Initialized)
		{
			OnInventoryStartedPostDrop();
		}
		else
		{
			Inventory->ComponentStarted.AddDynamic(this, &AA_ItemActor::OnInventoryStartedPostDrop);
		}
	}
}

void AA_ItemActor::OnInventoryStartedPostDrop()
{
	if(Inventory && Inventory->ContainerSettings.IsValidIndex(0) && Inventory->ContainerSettings[0].Items.IsValidIndex(0))
	{
		Inventory->RemoveTagFromItem(Inventory->ContainerSettings[0].Items[0], Inventory->EquipTag);
	}
}
