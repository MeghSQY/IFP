...
static FVector2D GetMenuOffsetForPlacement(const FGeometry& AllottedGeometry, float PopupPadding, EMenuPlacement PlacementMode, FVector2D PopupSizeLocalSpace, bool bIsContextMenu, FVector2D ContextMenuPosition)
{
	if (bIsContextMenu)
	{
	...
	switch (PlacementMode)
	{
		case MenuPlacement_BelowAnchor:
			return FVector2D(0.0f, AllottedGeometry.GetLocalSize().Y + PopupPadding);
			break;
		...
		case MenuPlacement_BelowRightAnchor:
			return FVector2D( -( PopupSizeLocalSpace.X + PopupPadding ) + ( AllottedGeometry.GetLocalSize().X ), AllottedGeometry.GetLocalSize().Y + PopupPadding );
			break;
		case MenuPlacement_ComboBox:
			return FVector2D(0.0f, AllottedGeometry.GetLocalSize().Y + PopupPadding);
			break;
		case MenuPlacement_ComboBoxRight:
			return FVector2D(AllottedGeometry.GetLocalSize().X - PopupSizeLocalSpace.X - PopupPadding, AllottedGeometry.GetLocalSize().Y + PopupPadding);
			break;
		case MenuPlacement_MenuRight:
			return FVector2D(AllottedGeometry.GetLocalSize().X + PopupPadding, 0.0f);
			break;
		case MenuPlacement_AboveAnchor:
			return FVector2D(0.0f, -PopupSizeLocalSpace.Y - PopupPadding);
			break;
		...
		case MenuPlacement_AboveRightAnchor:
			return FVector2D( -( PopupSizeLocalSpace.X + PopupPadding ) - ( AllottedGeometry.GetLocalSize().X ), -PopupSizeLocalSpace.Y - PopupPadding );
			break;
		case MenuPlacement_MenuLeft:
			return FVector2D(-PopupSizeLocalSpace.X - PopupPadding, 0.0f);
			break;
		...
		case MenuPlacement_RightLeftCenter:
			return FVector2D( AllottedGeometry.GetLocalSize().X + PopupPadding, - ( ( PopupSizeLocalSpace.Y / 2 ) - ( AllottedGeometry.GetLocalSize().Y / 2 ) ) );
			break;
...
SBCMenuAnchor::FPopupPlacement::FPopupPlacement(const FGeometry& PlacementGeometry, const FVector2D& PopupDesiredSize, float PopupPadding, EMenuPlacement PlacementMode, bool bIsContextMenu, FVector2D ContextMenuPosition)
{
	...
	LocalPopupOffset = GetMenuOffsetForPlacement(PlacementGeometry, PopupPadding, PlacementMode, LocalPopupSize, bIsContextMenu, ContextMenuPosition);
...
/*static*/ TArray<TWeakPtr<IMenu>> SBCMenuAnchor::OpenApplicationMenus;
 ...
 */
void SBCMenuAnchor::Construct( const FArguments& InArgs )
{
	...
	PopupPadding                           = InArgs._PopupPadding;
...
FGeometry SBCMenuAnchor::ComputeNewWindowMenuPlacement(const FGeometry& AllottedGeometry, const FVector2D& PopupDesiredSize, EMenuPlacement PlacementMode) const
{
	// Compute the popup size, offset, and anchor rect  in local space
	const FPopupPlacement PopupPlacement(AllottedGeometry, PopupDesiredSize, PopupPadding, PlacementMode, bIsContextMenu, ContextMenuPosition);
...
void SBCMenuAnchor::Tick( const FGeometry& AllottedGeometry, const double InCurrentTime, const float InDeltaTime )
{
	...
	else if (PopupWindow.IsValid() && IsOpenAndReusingWindow())
	{
		...
		const FPopupPlacement LocalPlacement(AllottedGeometry, Children[1].GetWidget()->GetDesiredSize(), PopupPadding, Placement.Get(), bIsContextMenu, ContextMenuPosition);
		...
		if (bFitInWindow)
		{
			...
			if (bIsContextMenu)
			{
				...
				FSlateRect PopupRect(LocalPlacement.LocalPopupOffset, LocalPlacement.LocalPopupOffset + LocalPlacement.LocalPopupSize);
				
				const bool bAdjustmentNeeded = PopupRect.IntersectionWith(WindowRectLocalSpace) != PopupRect;
				if (bAdjustmentNeeded)
				{
					const bool bFitsRight = LocalPlacement.LocalPopupOffset.X + LocalPlacement.LocalPopupSize.X < WindowRectLocalSpace.Right;
					if (!bFitsRight)
					{
						FittedPlacement.X = LocalPlacement.LocalPopupOffset.X - LocalPlacement.LocalPopupSize.X;
					}

					const bool bFitsBottom = LocalPlacement.LocalPopupOffset.Y + LocalPlacement.LocalPopupSize.Y < WindowRectLocalSpace.Bottom;
					if (!bFitsBottom)
					{
						FittedPlacement.Y = LocalPlacement.LocalPopupOffset.Y - LocalPlacement.LocalPopupSize.Y;
					}
				}
			}
			else
			{
				FSlateRect PaddingOffset = LocalPlacement.Orientation == Orient_Horizontal
					                           ? FSlateRect(-PopupPadding, 0, PopupPadding, 0)
					                           : FSlateRect(0, -PopupPadding, 0, PopupPadding);
				FittedPlacement = ComputePopupFitInRect(
					LocalPlacement.AnchorLocalSpace + PaddingOffset,
					FSlateRect(LocalPlacement.LocalPopupOffset,
					           LocalPlacement.LocalPopupOffset + LocalPlacement.LocalPopupSize),
	...
	if (MenuContent)
	{
		AnimationTime = FMath::Clamp(AnimationTime + InDeltaTime / TransitionDuration * (bIsDesiredOpen ? 1.f : -1.f),
		                             0.f, 1.f);
		...
		if (bIsDesiredOpen)
		{
			Opacity = FMath::InterpEaseInOut(0.f, 1.f, AnimationTime, 2.f);
		}
		else
		{
			Opacity = FMath::InterpEaseInOut(1.f, 0.f, 1.0f - AnimationTime, 2.f);
		...
		MenuContent->SetRenderOpacity(Opacity);
	}
}

void SBCMenuAnchor::FinishCloseImmediately()
{
	if (!bIsDesiredOpen && MenuContent)
	{
		AnimationTime = 0.f;
		MenuContent->SetRenderOpacity(0.f);
...
bool SBCMenuAnchor::ComputeVolatility() const
{
	return SPanel::ComputeVolatility() || IsOpen();
}

void SBCMenuAnchor::OnArrangeChildren(const FGeometry& AllottedGeometry, FArrangedChildren& ArrangedChildren) const
{
	...
	{
		const FPopupPlacement LocalPlacement(AllottedGeometry, Children[1].GetWidget()->GetDesiredSize(), PopupPadding,
		                                     Placement.Get(), bIsContextMenu, ContextMenuPosition);
		ArrangedChildren.AddWidget(AllottedGeometry.MakeChild(Children[1].GetWidget(), LocalPlacement.LocalPopupSize,
		                                                      FSlateLayoutTransform(LocalPopupPosition)));
	}
}

FVector2D SBCMenuAnchor::ComputeDesiredSize(float) const
{
...
FChildren* SBCMenuAnchor::GetChildren()
{
	return &Children;
}

int32 SBCMenuAnchor::OnPaint(const FPaintArgs& Args, const FGeometry& AllottedGeometry, const FSlateRect& MyCullingRect,
                             ...
                             bool bParentEnabled) const
{
	...
	if (ArrangedChildren.Num() > 0)
	{
		...
		{
			LayerId = FirstChild.Widget->Paint(Args.WithNewParent(this), FirstChild.Geometry, MyCullingRect,
			                                   OutDrawElements, LayerId + 1, InWidgetStyle,
			...
			if (PopupChild != nullptr)
			{
				if (bShouldDeferPaintingAfterWindowContent)
				{
					OutDrawElements.QueueDeferredPainting(
						FSlateWindowElementList::FDeferredPaint(PopupChild->Widget, Args, PopupChild->Geometry,
						                                        bApplyWidgetStyleToMenu
							                                        ? InWidgetStyle
							                                        : FWidgetStyle(), bParentEnabled));
				}
				else
				{
					const TSharedPtr<SWindow> PresentingWindow = PopupWindowPtr.Pin();
					...
					{
						PopupChild->Widget->Paint(Args.WithNewParent(this), PopupChild->Geometry,
						                          PresentingWindow->GetClippingRectangleInWindow(), OutDrawElements,
						                          LayerId + 1, bApplyWidgetStyleToMenu ? InWidgetStyle : FWidgetStyle(),
						                          ShouldBeEnabled(bParentEnabled));
					}
				}
			}
...
void SBCMenuAnchor::SetIsOpen(bool InIsOpen, const bool bFocusMenu, const int32 FocusUserIndex)
{
		...
		if (InIsOpen)
		{
			...
			if (MenuContent.IsValid())
			{
				...
				if (MyWidgetPath.IsValid())
				{
					...
					const FVector2D DesiredContentSize = MenuContentRef->GetDesiredSize();
					// @todo slate: This is ignoring any window border size!
					const EMenuPlacement PlacementMode = Placement.Get();
					...
					if (PlacementMode == MenuPlacement_ComboBox || PlacementMode == MenuPlacement_ComboBoxRight)
					{
						...
						NewWindowSize = FVector2D(FMath::Max(MyGeometry.Size.X, DesiredContentSize.X),
						                          DesiredContentSize.Y);
					}
					...
					if (bUseApplicationMenuStack)
					{
						if (MethodInUse.GetPopupMethod() == EPopupMethod::CreateNewWindow)
						{
							...
							TSharedPtr<IMenu> NewMenu = FSlateApplication::Get().PushMenu(
								AsShared(), MyWidgetPath, MenuContentRef, NewPosition, TransitionEffect, bFocusMenu,
								NewWindowSize, MethodInUse.GetPopupMethod(), bIsCollapsedByParent);
							...
							if (ensure(NewMenu.IsValid()))
							{
								...
								{
									UE_LOG(LogSlate, Error,
									       TEXT("!NewMenu->GetOwnedWindow().IsValid(), WidgetPath:\n%s"),
									       *MyWidgetPath.ToString());

									static const FName SMultiBoxWidgetTypeName = "SMultiBoxWidget";
									for (int32 WidgetIndex = MyWidgetPath.Widgets.Num() - 1; WidgetIndex >= 0; --
									     WidgetIndex)
									{
										if (MyWidgetPath.Widgets[WidgetIndex].Widget->GetType() ==
											SMultiBoxWidgetTypeName)
										{
											TSharedRef<SMultiBoxWidget> MultiBoxWidget = StaticCastSharedRef<
												SMultiBoxWidget>(MyWidgetPath.Widgets[WidgetIndex].Widget);
											...
											for (int32 BlockIndex = 0; BlockIndex < Blocks.Num(); ++BlockIndex)
											{
												...
												BlockText += FString::Printf(
													TEXT("%s (%d)%s"), *Block->GetExtensionHook().ToString(),
													(int32)Block->GetType(), bIsFinalBlock ? TEXT("") : TEXT(", "));
											}
								...
								if (NewMenu->GetOwnedWindow().IsValid())
								{
									...
									NewMenu->GetOnMenuDismissed().AddSP(this, &SBCMenuAnchor::OnMenuClosed);
									PopupWindowPtr = NewMenu->GetOwnedWindow();
								}
								else
								{
									UE_LOG(LogSlate, Error, TEXT(" Menu '%s' could not open '%s'"), *ToString(),
									       *MenuContentRef->ToString());
						...
						else
						{
							...
							TSharedRef<SBCMenuAnchor> SharedThis = StaticCastSharedRef<SBCMenuAnchor>(AsShared());
							...
							TSharedPtr<IMenu> NewMenu = FSlateApplication::Get().PushHostedMenu(
								...
								MethodInUse.GetShouldThrottle(), bIsCollapsedByParent);
							...
							if (bFocusMenu)
							{
								FSlateApplication::Get().SetUserFocus(FocusUserIndex, MenuContentRef,
					...
					else // !bUseApplicationMenuStack
					{
						...
						if (MethodInUse.GetPopupMethod() == EPopupMethod::CreateNewWindow)
						{
							...
							EOrientation Orientation =
								(TransitionEffect.SlideDirection == FPopupTransitionEffect::SubMenu)
									? Orient_Horizontal
									: Orient_Vertical;
							...
							const FVector2D ScreenPosition = FSlateApplication::Get().CalculatePopupWindowPosition(
								Anchor, ExpectedSize, bAutoAdjustForDPIScale, FVector2D::ZeroVector, Orientation);
							...
							TSharedRef<SWindow> NewMenuWindow = SNew(SWindow)
								...
								.ActivationPolicy(bFocusMenu
									                  ? EWindowActivationPolicy::Always
									                  : EWindowActivationPolicy::Never)
								[
									MenuContentRef
								];
							...
							TSharedPtr<IMenu> NewMenu = MakeShareable(
								new FBCMenuInWindow(NewMenuWindow, MenuContentRef, bIsCollapsedByParent));
							FSlateApplication::Get().AddWindowAsNativeChild(
								NewMenuWindow, MyWidgetPath.GetWindow(), true);
						...
						else
						{
							...
							TSharedRef<SBCMenuAnchor> SharedThis = StaticCastSharedRef<SBCMenuAnchor>(AsShared());
							TSharedPtr<IMenu> NewMenu = MakeShareable(
								new FBCMenuInHostWidget(SharedThis, MenuContentRef, bIsCollapsedByParent));
	...
	bIsDesiredOpen = InIsOpen;

	if (MenuContent)
	{
		MenuContent->SetVisibility(InIsOpen ? EVisibility::Visible : EVisibility::HitTestInvisible);
	}
}

void SBCMenuAnchor::OnMenuClosed(TSharedRef<IMenu> InMenu)
{
	bDismissedThisTick = true;
...
void SBCMenuAnchor::ResetPopupMenuContent()
{
...
bool SBCMenuAnchor::IsOpen() const
{
	return MethodInUse.IsSet() && PopupMenuPtr.IsValid();
}

bool SBCMenuAnchor::IsDesiredOpen() const
{
	return bIsDesiredOpen;
}

bool SBCMenuAnchor::ShouldOpenDueToClick() const
{
	return !IsOpen() && !bDismissedThisTick;
}

FVector2D SBCMenuAnchor::GetMenuPosition() const
{
	FVector2D Pos(0, 0);
...
void SBCMenuAnchor::SetMenuPlacement(TAttribute<EMenuPlacement> InMenuPlacement)
{
...
void SBCMenuAnchor::SetFitInWindow(bool bFit)
{
...
bool SBCMenuAnchor::HasOpenSubMenus() const
{
	bool Result = false;
	if (PopupMenuPtr.IsValid())
	{
...
TSharedPtr<SWindow> SBCMenuAnchor::GetMenuWindow() const
{
	return IsOpen() ? PopupWindowPtr.Pin() : TSharedPtr<SWindow>();
}

void SBCMenuAnchor::OnMenuDismissed()
{
	if (PopupMenuPtr.IsValid())
	{
...
bool SBCMenuAnchor::UsingApplicationMenuStack() const
{
	return bUseApplicationMenuStack;
}

/*static*/
void SBCMenuAnchor::DismissAllApplicationMenus()
{
